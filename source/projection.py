# ---------------------------------------- 
# file: projection.py
# methods: projection, projection_vertices
# author: coppermouse
# ----------------------------------------
...
def filter_projected_polygons_out_of_view( projected_polygons, colors, edges ):
...
# TODO: rename to projection_polygons
def projection( polygons, colors, offset, theta, projection_factor, projection_offset, near, view_mode, fog_color, edges, height ):
    ...
    nps = near_points = near_polygons.reshape( len( near_polygons )*4, 3 ) - ( *offset, height )
    rotate( near_points, (0,0,1), -theta)

    near_polygons = near_points.reshape( len( near_points )//4, 4, 3 )

    if view_mode == 0:
        projected_polygons = (near_polygons[:,:,:2]) * projection_factor * (-0.01,0.01) + projection_offset

        return projected_polygons, colors

    elif view_mode == 1:
        rotate( near_points, (1,0,0), 0.4)
        ...
        projected_polygons, colors = filter_projected_polygons_out_of_view( projected_polygons, colors, edges )
